...
def getvalue(mat):
    ...
    array_data = array_struct['temp']  # 取出需要的数字矩阵部分
...
def getresult(n, m, kenel, dizhi1):
    ...
    print('*********************************************************')
    pca = PCA(n_components=25, svd_solver='auto').fit(x_train)  # 要求维数相等
    print('保留的特征数:', pca.n_components_)
    energy = pca.explained_variance_ratio_
    total = 0
    for ele in range(0, len(energy)):
        total = total + energy[ele]
        print((round(energy[ele], 4)), end=" ")
    print()
    print('能量占比总和:', round(total, 4))
    print('*********************************************************')
...
def saveresult(num, PCA_n, kenel, dizhi2):
    ...
    workbook_path = os.path.join(result_path, ('KNN_' + str(kenel) + '_PCA_' + str(PCA_n) + '.xls'))
...
for PCA_n in [25]:
    ...
    # for kenel in [10, 30, 50, 70, 90, 110, 130, 150]:
        # for dizhi in ['adc/300', 'adc/43', 'fa/300', 'fa/43']:
        for dizhi in ['adc/300']:
            print(dizhi)
            print(PCA_n)
            print(kenel)
            data = []
